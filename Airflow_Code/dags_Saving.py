...
_s3_root_folder = 'bank_folder/Saving'
...
'''
- 적금 데이터 S3에 저장 , S3는 폴더 개념 X , Key(key가 폴더형태로 보일뿐)로 관리
...
def SavingDataToS3(**context):
    ...
    _fileName = f"적금_{_year}_{_month}_{_day}.csv"
    
    _contr = contr()
    
    if Run(_contr.SavingRun(_fileName,_year,_month,_day)):
        ...
        raise Exception(f"Failed to fetch SavingRun {_fileName} Data.")
    print("Saving S3 Key : ",_s3_key)
    context['ti'].xcom_push(key='s3key', value = _s3_key)


'''
- 적금 데이터 .csv를 S3에서 로딩 후 MySQL DB에 저장 , DB 연결 해제
...
- Xcom pull value 
    - s3에 저장된 csv파일 key 값
'''
def SavingDataS3ToDB(**context):
    _s3_key = context['ti'].xcom_pull(task_ids='SavingData_To_S3',key='s3key')
    ...
    _df[_df.columns[:5]] = _df[_df.columns[:5]].replace({'-':None})
    print(_df.head())
...
with DAG(
    ...
    _task1 = PythonOperator(
        task_id = 'SavingData_To_S3',
        python_callable=SavingDataToS3,
        provide_context=True
    )
    
    _task2 = PythonOperator(
        task_id="SavingData_To_DB",
        python_callable= SavingDataS3ToDB,
        provide_context=True
    )
