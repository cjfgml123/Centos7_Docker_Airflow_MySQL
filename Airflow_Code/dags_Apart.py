...
from chlee.Collector_5 import Controller as contr
...
_tableName = "ApartTable"

'''
- 아파트실거래 매매 데이터 S3에 저장 , S3는 폴더 개념 X , Key(key가 폴더형태로 보일뿐)로 관리
...
def ApartDataToS3(**context):
    ...
    _fileName = f"아파트실거래_{_year}_{_month}.csv"
    _contr = contr()
    if _contr.ApartRun(_year+_month,_fileName):
        _s3_key = f'{_s3_folder}/{_fileName}'
        _s3.upload_file(_fileName, _s3_bucket_name, _s3_key)
        print(f"{_fileName} 아파트 데이터 업로드 완료")
        if os.path.isfile(_fileName):
            os.remove(_fileName)
    else:
        raise Exception(f"Failed to fetch Apart Deal {_fileName} Data.")
    context['ti'].xcom_push(key='s3key', value = _s3_key)

'''
- 아파트실거래매매 데이터 .csv를 S3에서 로딩 후 MySQL DB에 저장 , DB 연결 해제
...
- Xcom pull value 
    - s3에 저장된 csv파일 key 값
'''
def ApartDataS3ToDB(**context):
    _s3_key = context['ti'].xcom_pull(task_ids='ApartData_To_S3',key='s3key')
    _s3 = boto3.client('s3', endpoint_url=_endpoint, aws_access_key_id=_access_key, aws_secret_access_key=_secret_key)
    ...
    #_query.CreateTable_ApartData()
    _query.InsertApartDFToTable(_df)
    _query.DisConnect()
...
with DAG(
    ...
    _task1 = PythonOperator(
        task_id = 'ApartData_To_S3',
        python_callable=ApartDataToS3,
        provide_context=True
    ) 

    _task2 = PythonOperator(
        task_id="ApartData_To_DB",
        python_callable= ApartDataS3ToDB,
        provide_context=True
    )
